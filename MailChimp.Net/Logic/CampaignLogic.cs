...
namespace MailChimp.Net.Logic
{
    ...
    {
        private const string BaseUrl = "campaigns";
        ...
        /// <returns></returns>
        public Campaign AddOrUpdate(Campaign campaign)
        {
            //Crate a web request with MC us1 URL
            string requestUrl = $"{BaseUrl}/{campaign.Id}";

            var jsonHttpWebRequest = this.CreateJsonWebRequest(requestUrl, "PATCH");

            //Get JSON content for the request
            string responseJsonContent = this.GetJsonResponseFromRequest(jsonHttpWebRequest, campaign);

            return JsonConvert.DeserializeObject<Campaign>(responseJsonContent);
        }
